...
- ✅ **Fleet LIVE (2026-09-23):** Control relocated into herdr (`HERDR_ENV=1`, pane w1:p2, tab w1:t2). Fleet: worktrees `~/Dev/PartyPanic-{backend,client,reviewer}` on branches `agent/backend-s1-frame` / `agent/client-ui-shell` / `agent/reviewer-checklist`; tabs+agents: **backend** (kilo, w1:p3), **client** (kilo, w1:p4), **reviewer** (opencode, w1:p5). Onboarding prompts dispatched from `docs/agents/*` charters. Permission policy: Operator chose **Always allow `/home/vsad/*`** (agents must read `~/RULES.md`). **PR #1 (`docs/REVIEW.md` checklist) APPROVED by Control + MERGED — full protocol proven: charter → work → gates → PR → CI green → Control merge.** In flight: client wrote `LobbyUI.luau`/`MockState.luau`, gates run, opening PR; backend executing S1 frame under Control's mapping decision (contract → `src/shared/` + approved `default.project.json` Shared line; placeholder → `src/server/rounds/`; top-level `src/rounds/` deferred to sprint 3 per Vision). ⚠️ Ops gotchas: (1) kilo ignores opencode config (banner, harmless — prompts are self-contained); (2) kilo's first-run welcome screen can silently swallow a prompt — verify `state_change_seq`/read before assuming delivery; (3) `gh run watch` exit 0 does NOT mean run success — read job results.
