...
- 🚧 **Fleet forming:** charters + discipline protocol at `docs/agents/` (Control / Backend / Client / Reviewer; branches `agent/<role>-<task>` → PR → Reviewer verdict → CI green → Control merge; per-agent git worktrees — shared checkout would collide). **Blocker:** this Control session runs OUTSIDE herdr (`HERDR_ENV` unset) → herdr's own skill forbids controlling the session from outside it; bootstrap requires Control relocated into a herdr pane (`opencode --session ses_f32552730ffeKYN3m7jafy6lRC`) or an Operator-run bootstrap script.
